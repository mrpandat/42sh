...
class TestCommand(unittest.TestCase):
    ...
    def test_01_simple(self):
        ...
        self.assertTrue(self.lib.read_list(node, clexer))

    def test_02_semis(self):
        node = self.lib.init_ast_node()
        command = b'myword || myword; myword || myword;'
        clexer = self.init_and_process_lexer(command)
        self.assertTrue(self.lib.read_list(node, clexer))

    def test_03_no_ending_separator(self):
        node = self.lib.init_ast_node()
        command = b'myword || myword; myword || myword'
        clexer = self.init_and_process_lexer(command)
        self.assertTrue(self.lib.read_list(node, clexer))

    def test_04_ands(self):
        node = self.lib.init_ast_node()
        command = b'myword || myword & myword || myword &'
        clexer = self.init_and_process_lexer(command)
        self.assertTrue(self.lib.read_list(node, clexer))

    def test_05_ands_and_ors(self):
        node = self.lib.init_ast_node()
        command = b'myword || myword; myword || myword &'
        clexer = self.init_and_process_lexer(command)
        self.assertTrue(self.lib.read_list(node, clexer))

    def test_06_wrong_commands(self):
        node = self.lib.init_ast_node()
        command = b'myword ||| myword; myword || myword &'
        clexer = self.init_and_process_lexer(command)
